...
public partial class Calendar : ContentView
	{
        ...
        List<StackLayout> innerStackView;
        ...
        public Calendar()
		{
			TitleLeftArrow = new CalendarButton
			{
				...
				//Text = "❰",
                ImageSource = ImageSource.FromFile("previous_arrow.png"),
			...
			TitleRightArrow = new CalendarButton
			{
                ...
                //Text = "❱",
                ImageSource = ImageSource.FromFile("next_arrow.png"),
                TextColor = Color.FromArgb("#c82727")
			};
            ...
            innerStackView = new List<StackLayout>(42);
        ...
        public static readonly BindableProperty ColorListProperty =
            BindableProperty.Create(nameof(ColorList), typeof(List<ColorData>), typeof(Calendar), new List<ColorData>(),
                                    propertyChanged: (bindable, oldValue, newValue) => (bindable as Calendar).ChangeColors());
        ...
        {
            get { return (List<ColorData>)GetValue(ColorListProperty); }
		...
		{
			if (newValue == oldValue) return;
			buttons.FindAll(b => b.IsEnabled && (!b.IsSelected || SelectedBackgroundColor != Colors.Gray)).ForEach(b => b.BackgroundColor = newValue);
		}
		...
		{
			if (newValue == oldValue) return;
			buttons.FindAll(b => b.IsEnabled && (!b.IsSelected || SelectedTextColor != Colors.Gray) && !b.IsOutOfMonth).ForEach(b => b.TextColor = newValue);
		}
		...
		{
			if (newValue == oldValue) return;
			buttons.FindAll(b => b.IsEnabled && (!b.IsSelected || SelectedTextColor != Colors.Gray) && !b.IsOutOfMonth).ForEach(b => b.FontAttributes = newValue);
		}
		...
		public static readonly BindableProperty DatesFontSizeProperty =
			BindableProperty.Create(nameof(DatesFontSize), typeof(double), typeof(Calendar), 15.0,
                ...
                for (var i = 0; i < ShowNumOfMonths; i++)
                {
                    ...
                    for (int r = 0; r < 6; r++)
                    {
                        for (int c = 0; c < 7; c++)
                        {
                            ...
                            {
                                innerStackView.Add(new StackLayout
                                {
                            ...
                            var s = innerStackView.Last();
                            // mainCalendar.Children.Add(b, c, r);
                            // mainCalendar.Children.Add(s, c, r);
                            mainCalendar.Add(b, c, r);
                            mainCalendar.Add(s, c, r);
                        }
                    }
        ...
        protected void ChangeCalendar(CalandarChanges changes)
        {
			...
			{
                    ColorList = await HelperMethods.GetAgendaColorList(StartDate.Date);
                    ...
                    if (buttons[i].Date?.Date == DateTime.Today.Date)
                    {
                        buttons[i].BorderWidth = 2;
                        buttons[i].BorderColor = Color.FromArgb(AppSettings.Current.Settings.ThemeColor);
                    }
                    else
                    {
                        buttons[i].BorderWidth = 0;
                        buttons[i].BorderColor = Colors.Transparent;
                    }
                        ...
                        for (int k = 0; k < innerStackView[i].Children.Count;k++)
                        {
                            Label label = (Label)innerStackView[i].Children.Where(x => x is Label).FirstOrDefault();
                            innerStackView[i].Children.Remove(label);
                        }
                        ColorData dateColors = new ColorData();
                        dateColors = ColorList.Where(x => x.Date.Date == buttons[i].Date.Value.Date).FirstOrDefault();
                        if (dateColors != null && dateColors.Color != null && dateColors.Color.Count > 0)
                        {
                            for (int j = 0; j < dateColors.Color.Count; j++)
                            {
                                if (innerStackView[i].Children.Count < dateColors.Color.Count)
                                {
                                    try
                                    {
                                        if (!string.IsNullOrEmpty(dateColors.Color[j]))
                                        {
                                            var nameLabel = new Label
                                            {
                                                ...
                                                FontSize = 20,
                                                HeightRequest = 30,
                                                TextColor = Color.FromArgb(dateColors.Color[j])
                                            };
                                            innerStackView[i].Children.Add(nameLabel);
                                        }
                                    }
                                    catch(Exception ex)
                                    {
