...
public class MiscellaneousPaymentForm : ViewModelBase
	{
        ...
        ObservableCollection<BindableInvoiceDetailsView> _invoiceDetailsList = new ObservableCollection<BindableInvoiceDetailsView>();
        ...
        List<BindablePaymentInvoiceList> _paymentInvoiceList = new List<BindablePaymentInvoiceList>();
        ...
        List<BindableProformaInvoiceDetails> SelectedList = new List<BindableProformaInvoiceDetails>();
        public List<int> SelectedPaymentId = new List<int>();

        public MiscellaneousPaymentForm(IMapper mapper, INativeServices nativeServices, INavigation navigation) : base(mapper, null, null)
        {
            _mapper = mapper;
        ...
        private async Task GetDetails()
        {
            try
            {
                ...
                OperationDetails result = await ApiHelper.GetObject<OperationDetails>(string.Format(TextResource.GetMiscellaneousPaymentUrl,AppSettings.Current.SelectedStudent.ItemId));
                if (result.Success)
                {
                    ...
                    AppSettings.Current.OnlinePaymentCurrencyRoundingDigits = data.PaymentSettings.CurrencyData.RoundingDigits;
        ...
        private async Task GetTaxReceiptData(BindableProformaInvoiceDetails bindableProformaInvoice)
        {
            try
            {
                ...
                BindableReceiptDetailView result = await ApiHelper.PostRequest<BindableReceiptDetailView>(string.Format(TextResource.GetTaxReceiptDetails, bindableProformaInvoice.StudentID, Convert.ToInt32(PaymentTypes.MiscellaneousPayment), false, bindableProformaInvoice.OrderId), AppSettings.Current.ApiUrl);
                if (result != null)
                {
