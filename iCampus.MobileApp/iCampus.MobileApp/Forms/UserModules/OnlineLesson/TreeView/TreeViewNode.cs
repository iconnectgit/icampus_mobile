...
public class TreeViewNode : StackLayout
    {
        ...
        private readonly BoxView _EmptyBox = new BoxView { BackgroundColor = Colors.White, Opacity = .5 };


        private const int ExpandButtonWidth = 5;
        ...
        internal readonly BoxView SelectionBoxView = new BoxView { Color = Colors.White, Opacity = .5, IsVisible = false };
        #endregion
        ...
        public TreeViewNode()
        {
            _nativeServices = Locator.Current.GetService<INativeServices>();
            ...
            _SpacerBoxView.GestureRecognizers.Add(_ExpandButtonGestureRecognizer);
            _ContentView.GestureRecognizers.Add(_ExpandButtonGestureRecognizer);
            ...
            Grid.SetRow(_ChildrenStackLayout, 1);         
            Grid.SetColumn(_ChildrenStackLayout, 0); 
        ...
        private void Render()
        {
            ...
            _SpacerBoxView.BackgroundColor = Colors.Transparent;
        ...
        private void ExpandButton_Tapped(object sender, EventArgs e)
        {
            try
            {
                ...
                if (view != null)
                {
                    if (view.Content is StackLayout stclayout)
                    {
                        var chl = stclayout.Children;
                        var lbl = chl.FirstOrDefault() as CustomLabel;
                        if (lbl != null)
                        {
                            if (lbl.FileType == FileTreeTypes.File)
                                _ = HelperMethods.OpenFileForPreview(lbl.FullPath, _nativeServices);
                        }
                    }
                }
