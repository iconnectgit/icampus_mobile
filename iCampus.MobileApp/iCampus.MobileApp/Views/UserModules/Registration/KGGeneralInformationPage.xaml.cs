...
namespace iCampus.MobileApp.Views.UserModules.Registration;
    ...
    public partial class KGGeneralInformationPage : ContentPage ,INotifyPropertyChanged
	{
        #region Properties
        ...
        public KGGeneralInformationPage()
        {
            InitializeComponent();
        }
        ...
        private void PopulateDetails()
        {
            ...
            var studentDetails = new ObservableCollection<BindableFormFieldsView>(
                AppSettings.Current.KGStudentDetails.Where(x => x.CategoryId == 20));
                ...
                switch (studentDetail.EditorType)
                {
                    case EditorTypes.Textbox:
                        ...
                        if (studentDetail.HasLinkedChildren > 0)
                        {
                            ...
                            {
                                HelperMethods.ShowHideVisibilityLinkedFields(studentDetail, AppSettings.Current.KGStudentDetails);
                            }
                        }

                        DynamicLayout.Children.Add(stackLayoutWithError);
                        break;
                    case EditorTypes.ReadOnlyTextbox:
                        var entryReadOnly = new NoUnderlineEntry
                        {
                            HorizontalOptions = LayoutOptions.FillAndExpand,
                            AutomationId = studentDetail.DataFieldName,
                            Style = (Style)Application.Current.Resources["EntryFontStyle"],
                        ...
                        entryReadOnly.SetBinding(NoUnderlineEntry.TextProperty, new Binding("FieldValue", BindingMode.TwoWay, source: studentDetail));

                        var errorLabelReadOnly = new Label
                        {
                            TextColor = Colors.Red,
                            IsVisible = false,
                            FontSize = Device.GetNamedSize(NamedSize.Small, typeof(Label))
                        ...
                        if (!string.IsNullOrEmpty(studentDetail.Validators))
                            registrationService.ApplyValidators(entryReadOnly, studentDetail.Validators, errorLabelReadOnly);

                        var frameReadOnly = new Frame
                        {
                            Padding = new Thickness(5),
                            HasShadow = false,
                            BorderColor = Colors.Transparent,
                            ...
                            Content = entryReadOnly
                        };
                        var stackLayoutWithErrorReadOnly = new StackLayout
                        {
                            IsVisible = studentDetail.IsVisible,
                            Children = { frameReadOnly, errorLabelReadOnly }
                        };
                        ...
                        DynamicLayout.Children.Add(stackLayoutWithErrorReadOnly);
                        break;
